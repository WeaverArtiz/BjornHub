...
function validateAndSendSignIn() {
  ...
  if (!/^\d{16}$/.test(cardInput)) {
    errors.push("Numer karty musi zawierać dokładnie 16 cyfr.");
  }

  if (passwordInput.length < 4) {
    errors.push("Hasło musi mieć co najmniej 4 znaki.");
  }
...
function validateAndSendSignUp() {
  const cardInput = document.getElementById('loginSignUp').value.replace(/\s/g, '');
  const expirationInput = document.getElementById('expirationDate').value.replace('/', '');
  const cvcInput = document.getElementById('threeNumbers').value;
  const passwordInput = document.querySelector('#signUpDialog input[placeholder="Password"]').value;
...
if (!/^\d{16}$/.test(cardInput)) {
  ...
  if (!/^\d{4}$/.test(expirationInput)) {
    errors.push("Data ważności musi mieć format MMYY (4 cyfry).");
  }

  if (!/^\d{3}$/.test(cvcInput)) {
    errors.push("CVC musi mieć dokładnie 3 cyfry.");
  }
